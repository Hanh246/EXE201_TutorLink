package com.exe201.tutorlink.main.plugin.mapper;

import com.exe201.tutorlink.common.plugin.AbstractMapperPlugin;
import com.exe201.tutorlink.main.dto.SubjectDTO;
import com.exe201.tutorlink.main.entity.Subjects;
import org.modelmapper.ModelMapper;
import org.springframework.stereotype.Component;

import java.util.List;

@Component
public class SubjectMapperPlugin extends AbstractMapperPlugin<Subjects, SubjectDTO, Long> {

    public SubjectMapperPlugin(ModelMapper mapper) {
        super(Subjects.class, SubjectDTO.class, Long.class, mapper);
    }
    @Override
    public List<String> getSearchableFieldNames() {
        return List.of("name");
    }

}
